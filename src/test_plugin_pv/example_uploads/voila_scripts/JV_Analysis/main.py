...
def load_files(file_list):
    ...
    for file_path in file_list:
        try:
            ...
            curves_merged = (
                pd.concat([curves_merged, jv_curve])
...
def jv_plot_curve_best(path, jvc, cur):
    ...
    fig.add_shape(
        type='line', x0=-0.2, y0=0, x1=1.35, y1=0, line=dict(color='gray', width=2)
    )
    fig.add_shape(
        type='line', x0=0, y0=-25, x1=0, y1=3, line=dict(color='gray', width=2)
    )
...
<br>PCE: {char_rev[3]:>5.1f}"""
    ...
    fig.add_annotation(
        x=0.24, y=-5, text=text_rev, showarrow=False, font=dict(size=12), align='left'
    )

    fig.add_annotation(
        x=0.55, y=-5, text=text_for, showarrow=False, font=dict(size=12), align='left'
    )
...
def jv_plot_by_cell_3x2(df, sample, path):
    ...
    measurement_cols = [
        col for col in all_cols if col not in metadata_cols and col != ''
    ]
        ...
        for direction in group['direction'].unique():
            for ilum in group['ilum'].unique():
                ...
                try:
                    ...
                    if (
                        abs(current_values.max()) < 0.1
                    ):  # If max current is less than 0.1, likely in A
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            hovertemplate='Voltage: %{x:.3f} V<br>Current Density: %{y:.3f} mA/cm²<br>Cell: '
                            + str(cell)
                            + '<br>'
                            + label,
                        ),
                    ...
                    print(
                        f'Debug: Error processing Cell {cell}, {direction}, {ilum}: {e}'
                    )
                ...
                try:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            hovertemplate='Voltage: %{x:.3f} V<br>Current Density: %{y:.3f} mA/cm²<br>'
                            + label,
                        )
                    )
                    print(f'Debug: Successfully added trace for {label}')
                    ...
                    print(
                        f'Debug: Available columns in voltage_row: {voltage_row.columns.tolist()}'
                    )
                    continue
...
def jv_plot_together(df1, df2, path, namestring):
    ...
    if namestring == 'All':
        ...
        df2_copy = pd.merge(sample_cells, df2, on=['sample', 'cell'], how='inner')[
            df2.columns
        ]
    ...
    source_data = df2_copy[
        ['sample', 'cell', 'direction', 'variable']
    ].drop_duplicates()
    source_data = (
        source_data[source_data['variable'] == 'Voltage (V)']
        if 'variable' in source_data.columns
        else source_data
    )
    ...
    for i in range(1, num_pairs + 1):
        if (
            i <= len(source_data)
            and not df2_plot[f'Voltage (V) {i}'].empty
            and not df2_plot[f'Current Density(mA/cm2) {i}'].empty
        ):
            # Get information about this curve
            if i <= len(source_data):
                sample_info = source_data.iloc[i - 1]
                sample_name = (
                    sample_info['sample'] if 'sample' in sample_info else f'Sample {i}'
                )
                cell_name = (
                    sample_info['cell'] if 'cell' in sample_info else f'Cell {i}'
                )
                direction = (
                    sample_info['direction']
                    if 'direction' in sample_info
                    else 'Unknown'
                )
            else:
    ...
    fig.update_layout(
        ...
        xaxis_title={'text': 'Voltage [V]', 'font': dict(size=14)},
        yaxis_title={'text': 'Current Density [mA/cm²]', 'font': dict(size=14)},
        xaxis=dict(range=[-0.2, 1.35]),
        yaxis=dict(range=[-25, 3]),
        template='plotly_white',
...
def boxplot_all_cells(path, wb, data, var_x, var_y, filtered_info, datatype):
    ...
    trash_counts = (
        trash.groupby(var_x)[var_name_y].count().to_dict() if not trash.empty else {}
    )
    ...
    title_text = f'Boxplot of {var_y} by {var_x}' + (
        ' (filtered out)' if datatype == 'junk' else ''
    )
    ...
    wb = save_combined_excel_data(
        path, wb, data, filtered_info, var_x, var_name_y, var_y, descriptor
    )
...
def boxplot_paired_by_direction(path, wb, data, var_x, var_y, filtered_info, datatype):
    ...
    if 'direction' not in data.columns:
        print(
            "Warning: 'direction' column not found in data. Creating dummy direction column."
        )
    ...
    if order_parameter != 'alphabetic':
        orderc = (
            data.groupby(var_x)[var_name_y]
            .describe()
            .sort_values(by=[order_parameter])['count']
            .index
        )
    else:
    ...
    trash_counts = (
        trash.groupby([var_x, 'direction'])[var_name_y].count().to_dict()
        if not trash.empty and 'direction' in trash.columns
        else {}
    )
    ...
    for category in orderc:
        for direction in directions:
            ...
            category_direction_data = data[
                (data[var_x] == category) & (data['direction'] == direction)
            ][var_name_y].dropna()
            ...
            if not category_direction_data.empty:
                ...
                fig.add_trace(
                    go.Box(
                        ...
                        line=dict(width=1.5),  # Slightly thicker lines
                        fillcolor=direction_colors.get(
                            direction, 'rgba(128, 128, 128, 0.7)'
                        ),
                        boxmean=True,  # Show mean
                        hoverinfo='all',
                        hovertemplate=(
                        ...
                        x=[f'{category}']
                        * len(
                            category_direction_data
                        ),  # Ensure proper x-axis positioning
                    )
                )
...
def save_combined_excel_data(
    path, wb, data, filtered_info, var_x, name_y, var_y, other_df
):
...
def plotting_string_action(plot_list, wb, data, supp, is_voila=False):
    ...
    for pl in plot_list:
        ...
        if 'B' in pl and var_x is not None and var_y is not None:
            ...
            fig, fig_name = jv_plot_together(
                filtered_jv, complete_cur, path, 'Filtered'
            )
            ...
            print(
                f"sample/cell combinations: {omitted_jv[['sample', 'cell']].drop_duplicates().shape[0]}"
            )
...
def ask_to_input_initial_folder():
    user_path = input(
        'Enter the path to the data folder for analysis. For example, C:\\Data\\Experiment : '
    )
