...
class MinimalistExperimentBuilder:
    ...
    def _create_process_row(self, index, process_data):
        ...
        if checkbox_controls:
            checkbox_row_elements = [
                widgets.HTML(
                    value='', layout=widgets.Layout(width='30px')
                ),  # Align with index
                widgets.HTML(
            ...
            return widgets.VBox(
                [main_row, checkbox_row], layout=widgets.Layout(margin='2px 0')
            )
        else:
            return main_row
    ...
    def _create_inline_config_controls(self, index, process_name, config):
        ...
        if process_name == 'Inkjet Printing':
            checkbox_options = [('annealing', 'Annealing'), ('gavd', 'GAVD')]
    ...
    def _update_process_type(self, index, new_process_type):
        ...
        if index < len(self.current_sequence):
            ...
            if new_process_type in [
                ...
                self.current_sequence[index]['config'] = self._get_default_config(
                    new_process_type
                )
            else:
    ...
    def _generate_excel_data(self):
        ...
        if EXCEL_BUILDER_AVAILABLE:
            try:
                ...
                with self.status_output:
                    print(
                        f'✅ ExperimentExcelBuilder created file: {len(excel_data)} bytes'
                    )
                    print(
            ...
            with self.status_output:
                print(
                    '❌ ExperimentExcelBuilder not available, using basic openpyxl...'
                )
        ...
        try:
            ...
            ws['A3'] = (
                f"Testing Mode: {'Yes' if self.is_testing_checkbox.value else 'No'}"
            )
...
"""
            for i, process in enumerate(self.current_sequence, 1):
                ...
                if 'config' in process and process['config']:
                    config_str = ', '.join(
                        [f'{k}={v}' for k, v in process['config'].items()]
                    )
