...
def get_all_uploads(url, token, number_of_uploads=20):
    response = requests.get(
        ...
        params=dict(
            page_size=number_of_uploads, order_by='upload_create_time', order='desc'
        ),
    )
...
def get_batch_ids(url, token, batch_type='INL_Batch'):
    query = {
        'required': {'data': '*'},
        'owner': 'visible',
    ...
    return [
        d['archive']['data']['lab_id'] for d in data if 'lab_id' in d['archive']['data']
    ]


def get_ids_in_batch(url, token, batch_ids, batch_type='INL_Batch'):
    query = {
        'required': {'data': '*'},
        'owner': 'visible',
...
def get_all_JV(url, token, sample_ids, jv_type='INL_JVmeasurement'):
    # collect the results of the sample, in this case it are all the annealing temperatures
...
def get_all_eqe(url, token, sample_ids, eqe_type='INL_EQEmeasurement'):
    query = {
        'required': {'metadata': '*'},
        'owner': 'visible',
        'query': {'results.eln.lab_ids:any': sample_ids},
...
def get_all_mppt(url, token, sample_ids, mppt_type='INL_SimpleMPPTracking'):
    query = {
        'required': {'metadata': '*'},
        'owner': 'visible',
        'query': {'results.eln.lab_ids:any': sample_ids},
...
def get_processing_steps(
    url, token, sample_ids, process_type='baseclasses.BaseProcess'
):
    ...
    response.raise_for_status()
    data = list(
        map(lambda process: process['archive']['data'], response.json()['data'])
    )
    ...
    return dict(
        map(
            lambda x: (
                ...
                x['archive']['results']['properties']['optoelectronic']['solar_cell'][
                    'efficiency'
                ],
            ),
            response.json()['data'],
